...
namespace DeploymentTrackerCore.Services.DeploymentManagement
{
    public class JenkinsDeploymentManager : IDeploymentManager
    {
        ...
        public Task<string> GetTeardownUrl(IDeployedSite deployment)
        {
            return Task.FromResult($"{Configuration.BaseUrl}/job/{Configuration.TeardownProject}/parambuild?{Configuration.TeardownSiteName}={deployment.SiteName}&{Configuration.TeardownDestroySql}=true");
        }
    }
