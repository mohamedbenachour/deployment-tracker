...
namespace IntegrationTests.Helpers {
    public static class TestApplicationFactoryExtensions {
        ...
        public static async Task<HttpClient> GetAuthenticatedClient(this TestApplicationFactory<DeploymentTrackerCore.Startup> factory, string userName = DefaultUserName) {
            var client = factory.WithWebHostBuilder(builder => {
                    builder.ConfigureTestServices(services => {
                        services.AddAuthentication("Test")
                            .AddScheme<MockAuthenticationOptions, MockAuthenticationHandler>(
                                "Test", options => {
                                    options.MockUserName = userName;
                                });
                    });
                })
